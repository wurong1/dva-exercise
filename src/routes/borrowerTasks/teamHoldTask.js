...
class TeamHoldTaskPage extends Component {
  state = {
    isReset: false,
    isFollowedUp: false,
      ...
      <div>
        ...
        >
          <Spin spinning={modal.loading}>
            <DeployModal key={holdDeployVisible} taskOptions="AUDITFOLLOWUP" taskType="GROUP_TASK" />
          </Spin>
        </Modal>
        <Modal
          title="转给分公司"
        ...
        >
          <Spin spinning={modal.loading}>
            <TransferModal key={holdTransferVisible} taskOptions="AUDITFOLLOWUP" taskType="GROUP_TASK" />
          </Spin>
        </Modal>
        <Modal
          title="分配签约人"
        ...
        >
          <Spin spinning={modal.loading}>
            <SignModal key={holdSignVisible} taskOptions="AUDITFOLLOWUP" taskType="GROUP_TASK" />
          </Spin>
        </Modal>
      </div>
    );
...
TeamHoldTaskPage.propTypes = {
};

export default connect(
  state => ({
    employeeGroupList: state.teamTasks.employeeGroupList,
    productcodeList: state.teamTasks.holdProductcodeList,
    searchList: state.teamTasks.holdTaskList,
    loading: state.teamTasks.holdTaskLoading,
  ...
  dispatch => ({
    ...
    getHoldProductcode: (guidedType) => {
      dispatch({ type: 'teamTasks/getHoldProductcode', payload: guidedType });
    },
    holdSearchListReset: () => {
      dispatch({ type: 'teamTasks/holdSearchListReset' });
    },
    holdTaskSearch: (formData, pageData) => {
      ...
      dispatch({ type: 'teamTasks/holdTaskSearch', payload: resultData });
    },
    getButtonConfig: parmas => dispatch({ type: 'teamTasks/getButtonConfig', payload: parmas }),
    taskHoldsetRows: parmas => dispatch({ type: 'teamTasks/taskHoldsetRows', payload: parmas }),
    showHoldDeployModal: parmas => dispatch({ type: 'teamTasks/showHoldDeployModal', payload: parmas }),
    showHoldTransferModal: parmas => dispatch({ type: 'teamTasks/showHoldTransferModal', payload: parmas }),
    showHoldSignModal: parmas => dispatch({ type: 'teamTasks/showHoldSignModal', payload: parmas }),
    taskHoldPageChange: parmas => dispatch({ type: 'teamTasks/taskHoldPageChange', payload: parmas }),
  }),
)(Form.create()(TeamHoldTaskPage));
