...
import { Row, Col, Button, Modal, message, Spin, Select, Steps, Tabs } from 'antd';
...
class LoanTaskDetail extends Component {
  ...
  showReviewModal = (flag) => {
    const {
      taskInfo: {
        loanBaseInfor: { loanAppStatusCode } = {},
      },
    } = this.props;
    if (flag && loanAppStatusCode !== 'CREATED' && loanAppStatusCode !== 'NEW') {
      message.error('当前贷款状态不能提交');
      return false;
    }
    this.props.showReviewModal(flag);
  }
  ...
  handleReview = () => {
    ...
    } else if (loanAppSource === 'MCA_ICRC_TEST') {
      configCode = 'MCA_ICRC_TEST';
    } else {
      configCode = 'crm';
    }
    const params = {
    ...
    this.props.reviewSubmit(params);
  }
  ...
  render() {
    ...
    const loanInfoData = {
      ...
      taskStatus,
      userPhone: phone,
      realLoanId: loanId, // 还款信息和签约上传文件需要
          ...
          <TabPane tab="任务详情" key="details">
            <div>
              ...
              <div style={{ display: showothers ? 'block' : 'none' }} className="crm-btn-group">
                ...
                <Button
                  onClick={this.handleButtonChange.bind(this, 'WITHDRAWAL')}
                  style={{ display: loanAppStatusCode === 'EXPIRED' ? 'none' : 'inline-block' }}
                >
                  取消录单
                </Button>
              ...
              <div style={{ display: showothers ? 'none' : 'block' }}>
                ...
                <div className="crm-btn-group">
                  { (loanAppStatusCode === 'CREATED' || loanAppStatusCode === 'NEW') && productCode !== 'PL-CSF-YYFQI01-019608-01' &&
                    <div style={{ display: 'inline-block' }}>
                      <Button
                        onClick={this.showReviewModal.bind(this, true)}
                      ...
                      <Button
                        ...
                        style={{ display: (loanType === 'SpeedLoan' || /_RELOAN/.exec(productCode)) ? 'none' : 'inline-block' }}
                      >
                        退回修改
                      </Button>
                    </div>
              ...
              <Modal
                ...
                onOk={this.handleReview.bind(this)}
                onCancel={this.showReviewModal.bind(this, false)}
                title="提交审核"
              >
                <Spin spinning={reviewloading}>
                  <p>确认提交到风控审核？</p>
                  <p>提交前，请确保客户贷款必填进件资料都已经填写。</p>
                </Spin>
              </Modal>
              <Modal
...
export default connect(
  ...
  dispatch => ({
    ...
    showReviewModal: params => dispatch({ type: 'loanDetail/showReviewModal', payload: params }),
