...
class TeamSignTaskPage extends Component {
  state = {
    isReset: false,
    isFollowedUp: false,
    ...
    return (
      <div>
        <div>
          ...
          <a target="_blank" rel="noopener noreferrer" className="link-contractor" href="/bcrm/#/teamContractor">签约人分配后查询</a>
        </div>
        <div>
          <Table
            dataSource={records}
        ...
        >
          <Spin spinning={modal.loading}>
            <DeployModal key={signDeployVisible} taskOptions="SIGN" taskType="GROUP_TASK" />
          </Spin>
        </Modal>
        <Modal
          title="转给分公司"
        ...
        >
          <Spin spinning={modal.loading}>
            <TransferModal key={signTransferVisible} taskOptions="SIGN" taskType="GROUP_TASK" />
          </Spin>
        </Modal>
        <Modal
          title="分配签约人"
        ...
        >
          <Spin spinning={modal.loading}>
            <SignModal key={signSignVisible} taskOptions="SIGN" taskType="GROUP_TASK" />
          </Spin>
        </Modal>
      </div>
    );
...
export default connect(
  state => ({
    ...
    searchList: state.teamTasks.signTaskList,
    loading: state.teamTasks.signTaskLoading,
  ...
  dispatch => ({
    ...
    signSearchListReset: () => {
      dispatch({ type: 'teamTasks/signSearchListReset' });
    },
    signTaskSearch: (formData, pageData) => {
      ...
      dispatch({ type: 'teamTasks/signTaskSearch', payload: resultData });
    },
    getButtonConfig: parmas => dispatch({ type: 'teamTasks/getButtonConfig', payload: parmas }),
    taskSignsetRows: parmas => dispatch({ type: 'teamTasks/taskSignsetRows', payload: parmas }),
    showSignDeployModal: parmas => dispatch({ type: 'teamTasks/showSignDeployModal', payload: parmas }),
    showSignTransferModal: parmas => dispatch({ type: 'teamTasks/showSignTransferModal', payload: parmas }),
    showSignSignModal: parmas => dispatch({ type: 'teamTasks/showSignSignModal', payload: parmas }),
    taskSignPageChange: parmas => dispatch({ type: 'teamTasks/taskSignPageChange', payload: parmas }),
  }),
)(Form.create()(TeamSignTaskPage));
