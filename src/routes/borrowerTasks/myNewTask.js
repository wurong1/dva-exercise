...
class MyNewTaskPage extends Component {
  state = {
    isReset: false,
    isFollowedUp: false,
      ...
      <div>
                  ...
                  <FormItem label="意向用户来源" >
                    {getFieldDecorator('intentionSourceType', {
                      ...
                      <Select>
                        <Option value="">全部</Option>
                        {
                          intentionsourceList && intentionsourceList.map((val, idx) => {
                            return <Option key={idx} value={val.code}>{val.name}</Option>;
                          })
                        }
                      </Select>,
                    )}
                  ...
                  <FormItem label="首次推荐贷款类型" >
                    {getFieldDecorator('firstRecommendPreLoanProduct', {
                      ...
                      <Select>
                        <Option value="">全部</Option>
                        {
                          firstRecommendList && firstRecommendList.map((val, idx) => {
        ...
        >
          <Spin spinning={modal.loading}>
            <DeployModal key={newDeployVisible} taskOptions="NEWREGIST" taskType="MINE_TASK" />
          </Spin>
        </Modal>
        <Modal
          title="转给分公司"
        ...
        >
          <Spin spinning={modal.loading}>
            <TransferModal key={newTransferVisible} taskOptions="NEWREGIST" taskType="MINE_TASK" />
          </Spin>
        </Modal>
        <Modal
          title="分配签约人"
        ...
        >
          <Spin spinning={modal.loading}>
            <SignModal key={newSignVisible} taskOptions="NEWREGIST" taskType="MINE_TASK" />
          </Spin>
        </Modal>
      </div>
    );
...
export default connect(
  state => ({
    ...
    searchList: state.myTasks.newTaskList,
    loading: state.myTasks.newTaskLoading,
    taskNew: state.myTasks.taskNew,
  ...
  dispatch => ({
    ...
    getIntentionsource: () => {
      dispatch({ type: 'myTasks/getIntentionsource' });
    },
    getFirstRecommend: () => {
      dispatch({ type: 'myTasks/getFirstRecommend' });
    },
    newSearchListReset: () => {
      dispatch({ type: 'myTasks/newSearchListReset' });
    },
    newTaskSearch: (formData, pageData) => {
      ...
      dispatch({ type: 'myTasks/newTaskSearch', payload: resultData });
    },
    getButtonConfig: parmas => dispatch({ type: 'myTasks/getButtonConfig', payload: parmas }),
    taskNewsetRows: parmas => dispatch({ type: 'myTasks/taskNewsetRows', payload: parmas }),
    showNewDeployModal: parmas => dispatch({ type: 'myTasks/showNewDeployModal', payload: parmas }),
    showNewTransferModal: parmas => dispatch({ type: 'myTasks/showNewTransferModal', payload: parmas }),
    showNewSignModal: parmas => dispatch({ type: 'myTasks/showNewSignModal', payload: parmas }),
    taskNewPageChange: parmas => dispatch({ type: 'myTasks/taskNewPageChange', payload: parmas }),
  }),
)(Form.create()(MyNewTaskPage));
