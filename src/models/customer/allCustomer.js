...
import { fetchGetEmployeeGroupId, fetchSearch, fetchGetEmployeeId, fetchDeploy } from '../../services/customer';

export default {
  namespace: 'allCustomer',
  ...
  effects: {
    ...
    * phoneDeploy({ payload }, { call, put, select }) {
      try {
        ...
        const searchData = yield select(state => state.allCustomer.phoneSearchParams);
    ...
    * deploy({ payload }, { call, put, select }) {
      try {
        ...
        const searchData = yield select(state => state.allCustomer.searchParams);
