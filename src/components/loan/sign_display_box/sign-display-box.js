...
class SignDisplayBox extends Component {
  ...
  getOssPath(path) {
    if (path && this.context.loanInfoData.ossDownloadMethod === 'FRONTEND') {
      this.setState(() => {
        remote({
          method: 'GET',
          url: `/borrower/getImageAbsolutePath?key=${path}`
        }).then(res => {
  ...
  getFileList(e) {
    ...
    remote({
      method: 'GET',
      url: '/borrower/getallloandocumentforsignbytype?aid=' + this.context.loanInfoData.aid + '&loanAppId=' + this.context.loanInfoData.loanId + '&type=' + this.props.boxInfo.type + '&docId=' + this.props.boxInfo.docId + '&taskStatus=' + this.context.loanInfoData.taskStatus + '&routingSystem=' + this.context.loanInfoData.routingSystem
    }).then((data) => {
      this.showModal();
  ...
  downloadFile(path, e) {
    ...
    remote({
      method: 'GET',
      url: `/borrower/getImageAbsolutePath?key=${newPath}`
    }).then(res => {
    ...
    remote({
      ...
      if (imgArray && imgArray.length) {
        ...
        if ($images.length) {
          ...
          $imgbox = $('<div></div>').attr('id', 'imgsGroup').css('display','none');
        }
        let imgs = imgArray;
        imgs.reverse();
        $.each(imgs, (idx, img) => {
          ...
          if(this.context.loanInfoData.ossDownloadMethod === 'FRONTEND') {
            remote({
              method: 'GET',
              url: `/borrower/getImageAbsolutePath?key=${imgPath}`
            }).then((res) => {
  ...
  render() {
    ...
    const noticePdfFilePath = '/borrower/signPdfFile?id=' + realLoanId + '&type=' + this.props.boxInfo.type + '&routingSystem=' + this.context.loanInfoData.routingSystem;
    const action = this.context.loanInfoData.routingSystem === 'ICRC' ? '/borrower/uploadLoanappIcrcFile' : '/borrower/uploaddocuments';
    const data = this.context.loanInfoData.routingSystem === 'ICRC' ? {'loanId': this.context.loanInfoData.loanId, 'conditionId': this.props.boxInfo.docId, 'status': this.context.loanInfoData.taskStatus} : {'aid': this.context.loanInfoData.aid, 'loanId': this.context.loanInfoData.realLoanId, 'docId': docId};

    return(
      type === 'SIGN_VIDEO' ?
        <div className="col-cell col-xs-12">
          <p>
