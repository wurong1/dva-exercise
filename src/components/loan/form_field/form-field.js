...
class FormField extends Component {
  ...
  handleChange(val) {
    ...
    if(this.context.loanInfoData.configProductCode === 'HIGH_SALARY') {
      if(this.props.fieldInfo.name === 'job_highSalaryClientType') {
  ...
  render() {
    ...
    case 'CHANNEL':
      input = getFieldDecorator(applyName, {
        ...
        <Select size="large" disabled={canNotEdit} showSearch optionFilterProp="children">
          {this.state.channelOptions && this.state.channelOptions.filter((val)=>val.channelId && val.channelName).map((val)=>{
            return <Option key={val && val.channelId} value={val && val.channelId && (val.channelId).toString()}>{val.channelName}</Option>;
          })}
        </Select>);
      break;
    case 'DATE':
    ...
    case 'TEXT_AREA':
      ...
      })(<TextArea name={name} disabled={canNotEdit} rows={4} className="la-textarea"/>);
      break;
    case 'PROVINCE':
      isProvinceRight ?
        input = getFieldDecorator(applyName, {
