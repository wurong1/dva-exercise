...
class DynamicForm extends Component {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      remote({
        ...
        if(ret.code === 20000) {
          remote({
            method: 'GET',
            url: '/borrower/loadFormInfo?aid=' + this.context.loanInfoData.aid + '&appId=' + this.context.loanInfoData.loanId + '&configCode=' + this.context.loanInfoData.code
          }).then((ret1) => {
            if(ret1.code === 20000) {
  ...
  commonFormSubmit() {
    const isPreAuthorize = this.context.loanInfoData.isPreAuthorize === 'true';
    this.props.form.validateFields((err, values) => {
      if (err) {
        msgBoxShow('您填写的信息有误！', 'faild', null);
        return;
  ...
  render() {
    ...
    const isIcrcControl = detailType === 'AUTOFLLOWUP' && this.context.loanInfoData.routingSystem === 'ICRC' && this.context.loanInfoData.loanStatus === 'HOLD';
              ...
              this.props.formInfo.fields.map((val, idx)=> {
                ...
                return (
                  <FormField key={idx} idx={index} fieldInfo={val} enterpriseFlag={this.state.enterpriseFlag}
                             ...
                             arry={this.state.arry} id={data.id} enterpriseData={this.state.enterpriseData}
      ...
      fields = this.props.formInfo.fields.map((val, idx)=> {
        ...
        return (
          <FormField key={idx} fieldInfo={val} fileInfo={this.props.fileInfo}  enterpriseFlag={this.state.enterpriseFlag}
                     ...
                     cities={this.state.cities} canNotEdit={canNotEdit} getChildChange={::this.getChildChange}
...
function fileConcept(type, img) {
  ...
  if (resultType === 'jpg' || resultType === 'png' || resultType === 'jpeg') {
    output = <img src={img} style={{width: '280px', height: '280px'}} />;
