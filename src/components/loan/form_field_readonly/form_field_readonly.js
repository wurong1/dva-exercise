...
class FormFieldReadonly extends Component {
  ...
  getMobileCarrierInfo(e) {
    ...
    if(!mcData) {
      remote({
        method: 'GET',
        url: '/borrower/getcreditreportfromoperator?phone=' + this.context.loanInfoData.userPhone + '&ssn=' + ssn
      }).then((data) => {
        let result = {};
        let status = '';
        try {
                ...
                {isChannel &&
                  getFieldDecorator(applyName, {
                    ...
                    <Select className="readonly-select" disabled>
                      {this.state.channelOptions && this.state.channelOptions.filter((v)=>v.channelId && v.channelName).map((opt)=> {
                        return <Option key={opt && opt.channelId} value={opt && opt.channelId && (opt.channelId).toString()}>{opt.channelName}</Option>;
                      })}
                    </Select>
                  )}
                {!isSelect && !isChannel &&
