...
class FileDisplayBox extends Component {
  ...
  deleteFile(record, e) {
    ...
    confirm({
      ...
      onOk() {
        remote({
          method: 'GET',
          url: '/borrower/deleteProfile?aid=' + this.context.loanInfoData.aid + '&docId=' + record.id,
        }).then((res) => {
          if(res.code === 20000) {
  ...
  getClumns() {
    ...
    const columns = [{
      ...
      key: 'action',
      render: (text, record) => {
        const isIcrcControl = this.context.loanInfoData.detailType === 'AUTOFLLOWUP' && this.context.loanInfoData.routingSystem === 'ICRC' && this.context.loanInfoData.loanStatus === 'HOLD' && this.context.loanInfoData.isReadonly === 'false';
        return(
          <span>
            {!record.uid &&
            <span>
              ...
              {this.context.loanInfoData.detailType === 'LOANAPPGUIDE' && this.context.loanInfoData.isReadonly === 'false' &&
    ...
    if (self.state.currentFileList && self.state.currentFileList.length) {
      ...
      $.each(imgs, (idx, img) => {
        if(this.context.loanInfoData.ossDownloadMethod === 'FRONTEND') {
          remote({
            method: 'GET',
            url: `/borrower/getImageAbsolutePath?key=${img.location}`
          }).then((res) => {
  ...
  render() {
    ...
    const imgPath = this.context.loanInfoData.ossDownloadMethod === 'FRONTEND' && !this.state.originPath ? this.state.imgPath : `/borrower/loanshowimage?path=${this.state.imgPath}`;
        ...
        <div className="col-cell-group row">
          ...
          <Modal title={name} visible={this.state.visible}
            ...
            <div className="t-center ">
              {this.context.loanInfoData.detailType === 'LOANAPPGUIDE' && this.context.loanInfoData.isReadonly === 'false' &&
              (this.context.loanInfoData.loanAppStatusCode === 'CREATED' || this.context.loanInfoData.loanAppStatusCode === 'NEW') && !(isLimit && imgLength >= 1) &&
