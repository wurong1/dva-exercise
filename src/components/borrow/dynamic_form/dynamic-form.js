...
class DynamicForm extends Component {
  ...
  componentWillMount() {
    const patt = /RELOAN/;
    const isReloanBankTab = patt.test(this.context.loanInfoData.loanType) && this.props.formInfo.name === 'PERSONAL_RELOAN_bankAccount';
    ...
    if(isReloanBankTab) {
      remote({
        method: 'GET',
        url: '/borrower/findLoanBankCard?aid=' + this.context.loanInfoData.aid
      }).then((data) => {
  ...
  getChildChange(val, name) {
    if(val === 'CHINA_TOP_500' || val === 'PUBLIC_COMPANY' || val === 'WORLD_TOP_500') {
      ...
      remote({
        method: 'GET',
        url: `/borrower/find-bank-by-type?actorId=${this.context.loanInfoData.aid}&bankAccountType=${val}`
      }).then((data) => {
  ...
  dealWithDesenseData(formValues){
    const values = { ...formValues };
    const { isFieldTouched } = this.props.form;
    console.log(isFieldTouched('person_mobilePhone'));
    for (const p in values) {
      if ((p in this.state) && !isFieldTouched(p)) {
  ...
  formSubmit() {
    ...
    this.props.form.validateFields((err, formValues) => {
      ...
      const values = this.dealWithDesenseData(formValues); // 处理被修改的脱敏数据
      ...
      remote({
        method: 'POST',
        url: '/borrower/commitProfile?aid=' + this.context.loanInfoData.aid + '&appId=' + this.context.loanInfoData.loanId + '&formCode=' + formId,
        data: result
      }).then((ret) => {
        if(ret.code === 20000) {
          remote({
            method: 'GET',
            url: '/borrower/load_form?aid=' + this.context.loanInfoData.aid + '&appId=' + this.context.loanInfoData.loanId + '&configCode=' + this.context.loanInfoData.code
          }).then((ret1) => {
            if(ret1.code === 20000) {
              ...
              this.setState({submitLoadingFlag: false});
              message.error(ret1.message);
            }
          }).catch(() => {
            this.setState({submitLoadingFlag: false});
          });
          // this.context.getOriginFormValue(); // 重新获取非脱敏值
        } else {
          this.setState({submitLoadingFlag: false});
  ...
  commonFormSubmit() {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      remote({
        ...
        this.setState({submitLoadingFlag: false});
        message.error(error.response.data.errorData.message);
      });
    });
  }
  ...
  render() {
    ...
    let contractArray;
    const isReadonly = this.context.loanInfoData.isReadonly;
    let fields = null;
    const loanAppStatusCode = this.context.loanInfoData.loanAppStatusCode;
    const isIcrcControl = detailType === 'AUDITFOLLOWUP' && this.context.loanInfoData.routingSystem === 'ICRC' && this.context.loanInfoData.loanStatus === 'HOLD';
    const flag = isReadonly === true || detailType === 'AUDITFOLLOWUP' || detailType === 'SIGN' || allReadonly;
                ...
                if (readonlyFlag) {
                  return (
                    <FormFieldReadonly key={idx} val={val} idx={index}
                      ...
                      originFieldValue={this.props.originFormValue && this.props.originFormValue[index] && this.props.originFormValue[index][val.name]}
                      desenseFieldValue={this.props.desenseFormValue && this.props.desenseFormValue[index] && this.props.desenseFormValue[index][val.name]}
        ...
        if(val.fieldType === 'FORM') {
          ...
          return (
            <SubForm formInfo={val} key={idx} formValue={formValue && formValue[val.name]}
                     ...
                     originFormValue={this.props.originFormValue && this.props.originFormValue[val.name]}
                     desenseFormValue={this.props.desenseFormValue && this.props.desenseFormValue[val.name]}
                     currentName={this.props.currentName}
            />
          );
        }
        if(readonlyFlag && !this.props.isCommonField) {
          return (
            <FormFieldReadonly val={val} key={idx} label={formValue && formValue[val.name]}
                               ...
                               originFieldValue={this.props.originFormValue && this.props.originFormValue[val.name]}
                               desenseFieldValue={this.props.desenseFormValue && this.props.desenseFormValue[val.name]}
                               currentName={this.props.currentName}
            />
          );
        }
    ...
    return(
      <div>
        <Spin spinning={this.context.loadingTab}>
            ...
            <div>
              ...
              <div className="row col-cell-group">
                <div className="col-cell tab-div">
                  ...
                  <span>{this.state.reloanBankInfo.accN && this.context.loanInfoData.cusName}</span>
                </div>
                <div className="col-cell tab-div">
...
//     appId: this.context.loanInfoData.loanId,
//     formCode: name
//   };
//   let url1;
