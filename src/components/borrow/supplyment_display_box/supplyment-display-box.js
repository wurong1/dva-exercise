...
class SupplymentDisplayBox extends Component {
  ...
  getFileList(e) {
    ...
    remote({
      method: 'GET',
      url: '/borrower/getallloanimageforauditbytype?aid=' + this.context.loanInfoData.aid + '&loanAppId=' + this.context.loanInfoData.loanId + '&type=' + this.props.boxInfo.type + '&taskStatus=hold' + '&routingSystem=' + this.context.loanInfoData.routingSystem + '&conditionId=' + this.props.boxInfo.docId + '&appStatus=' + this.context.loanInfoData.loanStatus,
    }).then((data) => {
      this.showModal();
  ...
  deleteFile(record, e) {
    ...
    confirm({
      ...
      onOk() {
        remote({
          method: 'DELETE',
          url: '/borrower/deleteborrowerdocument?aId=' + self.context.loanInfoData.aid + '&loanId=' + self.context.loanInfoData.loanId + '&docId=' + record.docId + '&docFileId=' + record.docFileId + '&type=' + record.type + '&taskStatus=hold' + '&routingSystem=' + self.context.loanInfoData.routingSystem + '&appStatus=' + self.context.loanInfoData.loanStatus,
        }).then((data) => {
  ...
  reUpload(uid, e) {
    ...
    const action = this.context.loanInfoData.routingSystem === 'ICRC' ? '/borrower/uploadLoanappIcrcFile' : '/borrower/uploadloanimages';
    const name = '{aid:' + this.context.loanInfoData.aid + ',' + 'loanId:' + this.context.loanInfoData.loanId + '}?file_' + type + '_0';
    ...
    if(this.context.loanInfoData.routingSystem === 'ICRC') {
      formData.append('loanId', this.context.loanInfoData.loanId);
            ...
            <span>
              {downloadMethod === 'FRONTEND' ?
                ...
                {record.readonly ? null :
                  <a className="btn btn-default-b modal-a-delete" onClick={(e) => this.deleteFile(record, e)}>删除</a>}
            </span>}
          {record.status === 'error' &&
            <span>
  ...
  render() {
    ...
    const action = this.context.loanInfoData.routingSystem === 'ICRC' ? '/borrower/uploadLoanappIcrcFile' : '/borrower/uploadloanimages';
    const data = this.context.loanInfoData.routingSystem === 'ICRC' ? {'loanId': this.context.loanInfoData.loanId, 'conditionId': this.props.boxInfo.docId, 'status': 'hold', 'appStatus': this.context.loanInfoData.loanStatus } : {'aid': this.context.loanInfoData.aid};
    const requiredClass = this.state.required === false ? 'cell-label file-box' : 'hold-required cell-label file-box';
    return(
        ...
        >
          ...
          <div className="t-center" style={{ textAlign: 'center', marginTop: '15px' }}>
            {fileGroup !== 'FILL_SUBMIT' ?
              this.context.loanInfoData.detailType === 'AUDITFOLLOWUP' &&
                <label className="label label-warning" style={{ color: '#e49533' }}>补件状态中处理结果选择“补件”，才可以做补件操作</label>
              :
                <FileUpload rules={['jpg', 'jpeg', 'png', 'gif', 'pdf', 'zip', 'rar', 'doc',
                  'docx', 'xlsx', 'xls']}
                            typePrompt="jpg, jpeg, png, gif, pdf, zip, rar, word, excel"
                            name={'{aid:' + this.context.loanInfoData.aid + ',' + 'loanId:' + this.context.loanInfoData.loanId + '}?file_' + type + '_0'}
                            action= {action}
                            data={data}
...
SupplymentDisplayBox.contextTypes = {
