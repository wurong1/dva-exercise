...
class SignDisplayBox extends Component {
  ...
  getFileList(e) {
    ...
    remote({
      method: 'GET',
      url: '/borrower/getallloandocumentforsignbytype?aid=' + this.context.loanInfoData.aid + '&loanAppId=' + this.context.loanInfoData.loanId + '&type=' + this.props.boxInfo.type + '&docId=' + this.props.boxInfo.docId + '&taskStatus=' + this.context.loanInfoData.taskStatus + '&routingSystem=' + this.context.loanInfoData.routingSystem + '&appStatus=' + this.context.loanInfoData.loanStatus,
    }).then((data) => {
      this.showModal();
  ...
  reUpload(uid, e) {
    ...
    const action = this.context.loanInfoData.routingSystem === 'ICRC' ? '/borrower/uploadLoanappIcrcFile' : '/borrower/uploaddocuments';
    const name = '{aid:' + this.context.loanInfoData.aid + ',' + 'loanId:' + this.context.loanInfoData.realLoanId + '}?file_' + type + '_0';
    ...
    if(this.context.loanInfoData.routingSystem === 'ICRC') {
      formData.append('loanId', this.context.loanInfoData.aid);
      formData.append('conditionId', this.props.boxInfo.docId);
      formData.append('status', this.context.loanInfoData.taskStatus);
      formData.append('appStatus', this.context.loanInfoData.loanStatus);
    }else {
      formData.append('aid', this.context.loanInfoData.aid);
      formData.append('loanId', this.context.loanInfoData.realLoanId);
  ...
  render() {
    ...
    const noticePdfFilePath = '/borrower/signPdfFile?id=' + this.context.loanInfoData.realLoanId + '&type=' + this.props.boxInfo.type + '&routingSystem=' + this.context.loanInfoData.routingSystem;
    const action = this.context.loanInfoData.routingSystem === 'ICRC' ? '/borrower/uploadLoanappIcrcFile' : '/borrower/uploaddocuments';
    const icrcData = {
      'loanId': this.context.loanInfoData.loanId,
      'conditionId': this.props.boxInfo.docId,
      'status': this.context.loanInfoData.taskStatus,
      'appStatus': this.context.loanInfoData.loanStatus,
    };
    const crcData = {
      'aid': this.context.loanInfoData.aid,
      'loanId': this.context.loanInfoData.realLoanId,
      'docId': docId,
    };
    const data = this.context.loanInfoData.routingSystem === 'ICRC' ? icrcData : crcData;
    return(
      type === 'SIGN_VIDEO' ?
        <div className="col-cell col-xs-12 sign-video" style={{ paddingLeft: '0px' }}>
          <p>
            <label className="sign-required cell-label">{name} (0/0)</label>
          </p>
          <div className="cell-textarea">
            <TextArea
              rows={4}
              onChange={(e) => { this.context.setSignVideoValue(e.target.value); }}
              style={{ height: 'auto' }}
            />
          </div>
        </div>
            ...
            <div className="col-cell-group row">
              <div className="col-cell col-xs-12 imgItem-search">
                {imgLength <= 0 ?
                  <div>
                    <p>
                      <label className="sign-required cell-label file-box" >{name}(0/0)</label>
                    ...
                    :
                      <div>
                        <p>
                          <label className="sign-required cell-label file-box" >{name}(1/{imgLength}) </label>
                          <a className="papers-box-link file-box" onClick={(e)=>this.getFileList(e)}>查看全部</a>
                            {type === 'LOAN_NOTICE' ?
                              <a href={noticePdfFilePath} target="_blank" className="btn btn-normal-b btn-check">查看模板</a>
                                : null}
              ...
              >
                ...
                <div className="t-center" style={{ textAlign: 'center', marginTop: '15px' }}>
                  {fileGroup !== 'CUSTOMER_APPROVAL' || this.context.loanInfoData.loanStatus !== 'APPROVED' ?
                    this.context.loanInfoData.detailType === 'SIGN' &&
                      <label className="label label-warning" style={{ color: '#e49533' }}>准备签约状态中处理结果选择“客户确认”，才可以做签约操作</label>
                      :
                        (type !== 'LITE' || this.context.loanInfoData.routingSystem === 'CRC') && <FileUpload rules={['jpg', 'jpeg', 'png', 'gif', 'pdf', 'zip', 'rar', 'txt']}
                                    typePrompt="jpg, jpeg, png, gif, pdf, zip, rar, txt"
                                    name={'{aid:' + this.context.loanInfoData.aid + ',' + 'loanId:' + this.context.loanInfoData.realLoanId + '}?file_' + type + '_0'}
                                    action= {action}
                                    data={data}
