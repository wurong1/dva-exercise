...
import {Form, Input, Select, Checkbox, DatePicker, Cascader, message } from 'antd';
...
class FormField extends Component {
  ...
  componentWillMount() {
    ...
    if(this.context.loanInfoData.configProductCode === 'HIGH_SALARY') {
  ...
  componentDidMount() {
    ...
    // const applyName = this.getApplyName();
    // const isPhone = this.props.fieldInfo.fieldType === 'PHONE' || this.props.fieldInfo.fieldType === 'MOBILE';
    // if (isPhone && this.props.originFieldValue) {
    //   this.context.setApplyName(applyName, this.props.originFieldValue);
    // }
  ...
  componentWillReceiveProps(nextProps) {
    ...
    // const applyName = this.getApplyName();
    // const nextApplyName = this.getNextApplyName(nextProps);
    // const currentName = nextProps.currentName; // 当前脱敏字段
    if (this.props.referCode !== nextProps.referCode && nextProps.referCode) {
      this.getChannelOptions(nextProps.referCode);
    }
    // 更新所有的脱敏字段值
    // if (nextProps.originFieldValue !== this.props.originFieldValue && nextProps.originFieldValue && isPhone) {
    //   this.context.setApplyName(applyName, nextProps.originFieldValue);
    // }
    // 多表单保存后applyName跟新
    // if (nextApplyName !== applyName && nextProps.originFieldValue && isPhone) {
    //   this.context.setApplyName(nextApplyName, nextProps.originFieldValue);
    // }
    // 设置当前操作的脱敏字段value
    // if (nextProps.originFieldValue !== this.props.originFieldValue && nextProps.originFieldValue && isPhone && applyName === currentName) {
    //   setFieldsValue({ [applyName]: nextProps.originFieldValue });
    // } else if (nextProps.desenseFieldValue !== this.props.desenseFieldValue && nextProps.desenseFieldValue && isPhone && applyName === currentName) {
    //   setFieldsValue({ [applyName]: nextProps.desenseFieldValue });
    // }
  }
  ...
  getNextApplyName(nextProps) {
    const isSubForm = nextProps.isSubForm;
    const isMultiple = nextProps.multiple;
    const subFormName = nextProps.subFormName;
    let applyName = nextProps.fieldInfo.name;
    if (isSubForm) {
      applyName = `${subFormName}_${applyName}`;
    }
    if (isMultiple) {
      const idx = nextProps.idx;
      applyName = `${applyName}_${idx}_${nextProps.id}`;
    }
    return applyName;
  }
  ...
  validationRules(validate, otherRule, initialData, callback) {
    ...
    // const fieldType = this.props.fieldInfo.fieldType; // field type是PHONE和MOBILE进行数据脱敏展示
    // const isMobile = fieldType === 'MOBILE';
    // const isPhone = fieldType === 'PHONE';
    // const applyName = this.getApplyName();
    // const isModified = isFieldTouched(applyName);
    let result = null;
    ...
    // if (isMobile && result !== 'success' && !isModified) {
    //   const mobilePatten = /^1[34578][0-9]\*{4}[0-9]{4}$/;
    //   result = mobilePatten.test(data) ? 'success' : result;
    // }
    // if (isPhone && result !== 'success' && !isModified) {
    //   const phonePatten = /^0[0-9]{2,3}-[0-9]{4,5}\*{3}$/;
    //   result = phonePatten.test(data) ? 'success' : result;
    // }
  ...
  getChannelOptions(referCode) {
    ...
    if(isChannel) {
      remote({
        ...
        message.error(e.response && e.response.data && e.response.data.errorData && e.response.data.errorData.message);
      });
    }
  }
  ...
  formatAddress(addr) {
    const source = this.context.loanInfoData.configCode;
  ...
  showValue = (applyName) => {
    const { setFieldsValue } = this.props.form;
    const { showOriginValue } = this.state;
    const { originFieldValue, desenseFieldValue } = this.props;
    if (showOriginValue) {
      this.context.getDesenseFormValue(applyName);
      setFieldsValue({ [applyName]: desenseFieldValue });
    } else {
      this.context.getOriginFormValue(applyName);
      setFieldsValue({ [applyName]: originFieldValue });
    }
    this.setState(prevState => ({
      showOriginValue: !prevState.showOriginValue,
    }));
    // const desenseValue = getFieldValue(applyName);
    // if (showOriginValue) {
    //   setFieldsValue({ [applyName]: this.state.desenseValue });
    // } else {
    //   setFieldsValue({ [applyName]: originValue || this.props.originFieldValue });
    //   this.context.getOriginFormValue(applyName);
    // }
    // this.setState(prevState => ({
    //   showOriginValue: !prevState.showOriginValue,
    //   desenseValue: showOriginValue ? prevState.desenseValue : desenseValue,
    // }));
  }
  ...
  render() {
    ...
    const addressData = (isAddress && value) ? this.formatAddress(value) : null;
    const readOnlyAddress = addressData ? addressData.join('/') : null;
    const isProvinceRight = (isProvince && isRightProvince(value, province)) || this.props.isCommonField;
    const isCityRight = (isCity && isRightCity(value, city)) || this.props.isCommonField;
    ...
    // 预授贷款类型
    const isPreAuthorize = this.context.loanInfoData.isPreAuthorize;
    ...
    case 'BANK':
      input = getFieldDecorator(applyName, {
          ...
          rules: [{
            ...
            <Select size="large" disabled={canNotEdit} showSearch optionFilterProp="children">
              {
                bankOptions && bankOptions.map((val, idx) => {
                  return <Option value={val && val.bankName} key={idx}>{val && val.bankName}</Option>
                })
              }
            </Select>);
      break;
    ...
    case 'CHECK':
      ...
      break;
    case 'DOCUMENT':
      input = <FileDisplayBox tabName={this.props.formInfo && this.props.formInfo.name} fieldInfo={this.props.fieldInfo} fileInfo={this.props.fileInfo}/>;
      break;
              ...
              <FormItem
                label={
                  <span>
                    {label}
                    {/*
                      (fieldType === 'PHONE' || fieldType === 'MOBILE') &&
                      <span className={ this.props.originFieldValue ? (showOriginValue ? 'icon-showpsw' : 'icon-closepsw') : ''} onClick={this.showValue.bind(this, applyName)} style={{ marginLeft: '8px'}}/>
                    */}
                  </span>
                }
                className={isRequired ? `required ${this.props.isCommonField ? 'common-required' : ''}` : ''}
                hasFeedback={false} 
              >
                {input}
              </FormItem>
              {isAddress && !canNotEdit &&
                <FormItem label={label + '_详细地址'}  className={isRequired ? `required ${this.props.isCommonField ? 'common-required' : ''}` : ''} hasFeedback={false}>
...
FormField.contextTypes = {
  ...
  setApplyName: React.PropTypes.func,
};