...
import {Input, Select, Modal, Row, Col, Table, message } from 'antd';
...
class FormFieldReadonly extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    this.setState({ originValue: nextProps.originFieldValue });
  }
  ...
  getMobileCarrierInfo(e) {
    ...
    if(!mcData) {
      ...
      remote({
        method: 'GET',
        url: `/borrower/v1/customer/phone?actorId=${this.context.loanInfoData.aid}`,
      }).then((res) => {
        const phoneNo = res && res.data;
        remote({
          method: 'GET',
          url: '/borrower/getcreditreportfromoperator?phone=' + phoneNo + '&ssn=' + ssn,
        }).then((data) => {
          let result = {};
          let status = '';
          try {
  ...
  showValue = (applyName) => {
    ...
    const { showOriginValue, originValue } = this.state;
  ...
  render() {
    ...
    const fieldType = val.fieldType; // field type是PHONE和MOBILE进行数据脱敏展示
    ...
    if(isAddress) {
      ...
      label = <FileDisplayBox tabName={this.props.formInfo && this.props.formInfo.name} fieldInfo={val} fileInfo={this.props.fileInfo}/>;
    }
    if(label === 0) {
              ...
              <p>
                ...
                </label>
                {
                  (fieldType === 'PHONE' || fieldType === 'MOBILE') &&
                    <span
                      className={label ? (showOriginValue ? 'icon-showpsw' : 'icon-closepsw') : ''}
                      onClick={this.showValue.bind(this, applyName)}
